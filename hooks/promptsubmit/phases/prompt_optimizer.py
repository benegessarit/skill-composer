...
Patterns supported:
    #pref question       → invoke mode-preflight (lightweight)
    #pref* question      → invoke mode-preflight with depth_probe
...
# === SKILL LOCATIONS ===
# Plugin-relative skills (for development/testing) or installed location
_PLUGIN_ROOT = Path(__file__).parent.parent.parent.parent
_PLUGIN_SKILLS = _PLUGIN_ROOT / "skills"
_INSTALLED_SKILLS = Path.home() / ".claude/skills"

# Prefer plugin-local skills, fall back to installed location
SKILLS_DIR = _PLUGIN_SKILLS if _PLUGIN_SKILLS.exists() else _INSTALLED_SKILLS
EXEC_OPT_PATH = SKILLS_DIR / "exec-opt/SKILL.md"

# === ACTION MODE DEFINITIONS ===
# Maps mode key to (trigger_pattern, skill_name, is_deep)
# is_deep=True means mode includes depth_probe call
ACTION_MODES = {
    # Standard modes (lightweight) - memorable short triggers
    "preflight": (r"#pref(?!\*)", "mode-preflight", False),
    "verify": (r"#ver(?!\*)", "mode-verify", False),
    "clarify": (r"#clar(?!\*)", "mode-clarify", False),
    ...
    "clarify_deep": (r"#clar\*", "mode-clarify", True),
...
# Extract hashtag modes from text (matches #pref, #pref*, #ver, #qheavy, etc.)
...
def check(ctx: dict) -> dict | None:
    ...
    Patterns:
        #pref question        → invoke mode-preflight
